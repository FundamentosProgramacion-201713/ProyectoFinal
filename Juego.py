...
def fin():
    ...
    if ganando > 0:
        ...
        texto4 = fuente.render("Correct", 1, NEGRO)
        ventana.blit(texto4, (100, 200))
        ganando -= 1
    elif perdiendo > 0:
        fuente = pygame.font.Font(None, 150)
        texto4 = fuente.render("YOU LOSE", 1, NEGRO)
        ventana.blit(texto4, (100, 200))
        fuente = pygame.font.Font(None, 50)
        texto4 = fuente.render("The Answere is: " + letras[0], 1, NEGRO)
...
def menu():
    pygame.draw.rect(ventana, BLANCO, (350, 250, 100, 100), 1)
    pygame.draw.polygon(ventana, BLANCO, ((355, 255), (355, 345), (445, 300)))
    pygame.draw.rect(ventana, BLANCO, (290, 275, 50, 50), 1)
    fuente = pygame.font.Font(None, 70)
    texto4 = fuente.render("#", 1, BLANCO)
    ventana.blit(texto4, (295, 280))
...
def juego():
    ...
    for i in range(0, len(respuesta)):
        x = ANCHO / 2 - 10 - (30 * (len(palabra) - 1)) + (60 * i)
        fuente = pygame.font.Font(None, 50)
        texto = fuente.render(respuesta[i][0], 1, NEGRO)
        ventana.blit(texto, (x, 60))
    for i in range(0, len(errores)):
        x = 20 * i
        fuente = pygame.font.Font(None, 50)
        texto = fuente.render(errores[i], 1, NEGRO)
        ventana.blit(texto, (x + 10, ALTO - 50))
    if error >= 1:
        pygame.draw.circle(ventana, NEGRO, (400, 200), 50, 1)
    if error >= 2:
        pygame.draw.line(ventana, NEGRO, (400, 250), (400, 350))
    if error >= 3:
        pygame.draw.line(ventana, NEGRO, (350, 300), (400, 300))
    if error >= 4:
        pygame.draw.line(ventana, NEGRO, (400, 300), (450, 300))
    if error >= 5:
        pygame.draw.line(ventana, NEGRO, (400, 350), (350, 400))


def puntaje():
    fuente = pygame.font.Font(None, 50)
    texto = fuente.render("Max Highscores", 1, NEGRO)
    ventana.blit(texto, (10, 60))
    for i in range(0, len(listpuntaje)):
        y = 100 + (50 * i)
        texto = fuente.render(listpuntaje[i], 1, NEGRO)
        ventana.blit(texto, (20, y))
    pygame.draw.rect(ventana, BLANCO, (630, 550, 160, 40), 1)
    fuente = pygame.font.Font(None, 50)
    texto = fuente.render("Regresar", 1, NEGRO)
...
for linea in puntajes:
    ...
    for i in range(0, len(cadena)):
        listpuntaje.append(cadena[i])
listpuntaje = sorted(listpuntaje, reverse=True)
puntajes.close()
